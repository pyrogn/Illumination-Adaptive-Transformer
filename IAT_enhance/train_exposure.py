...
from utils import PSNR, validation, LossNetwork, get_dist_info
...
parser.add_argument(
    "--img_path",
    type=str,
    default="/data/unagi0/cui_data/light_dataset/Exposure_CVPR21/train/INPUT_IMAGES",
)
parser.add_argument(
    "--img_val_path",
    type=str,
    default="/data/unagi0/cui_data/light_dataset/Exposure_CVPR21/validation/INPUT_IMAGES",
)
parser.add_argument(
    "--normalize",
    action="store_true",
    help="Default not Normalize in exposure training.",
)
...
parser.add_argument(
    "--snapshots_folder", type=str, default="workdirs/snapshots_folder_exposure"
)
...
val_dataset = exposure_loader(
    images_path=config.img_val_path, mode="val", normalize=config.normalize
)
...
for epoch in range(config.num_epochs):
    ...
    if rank == 0:
        ...
        if SSIM_mean > ssim_high:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(config.snapshots_folder, "best_Epoch" + ".pth"),
            )
